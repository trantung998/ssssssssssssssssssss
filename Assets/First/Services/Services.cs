    ...
    public abstract class MonoBehaviourService<T> : MonoBehaviour where T : class
    {
        void Awake()
        {
            ...
            if (attrs.Length > 0)
            {
                ...
                for (; i >= 0; i--)
                {
                    if (System.Text.RegularExpressions.Regex.IsMatch(sceneName, ((MonoBehaviourServiceFilterAttribute) attrs[i]).Name))
                    {
                        break;
                    }
                }
                ...
                if (i == -1)
                {
                    throw new UnityException(string.Format("\"{0}\" service cant be used at scene \"{1}\"", type.Name, sceneName));
                }
            }
#endif
    ...
    public static class Service<T> where T : class
    {
        ...
        public static T Get()
        {
            ...
            if (type.IsSubclassOf(typeof(ScriptableObject)))
            {
                ...
                {
                    throw new UnityException(string.Format("Service<{0}>.Get() can be used only with exists / loaded asset of this type", type.Name));
                }
        ...
        public static void Register(T instance)
        {
            if (IsRegistered)
            {
                throw new UnityException(string.Format("Cant register \"{0}\" as service - type already registered", typeof(T).Name));
            }
