namespace Gameplay.Services
{
    ...
    public sealed class ObjectPool : MonoBehaviour
    {
        ...
        private class ObjectAndDelayRecycle
        {
            public GameObject GameObjectRecycle;
            public float DelayTime;
        }

        private List<ObjectAndDelayRecycle> _delayRecycle = new List<ObjectAndDelayRecycle>();

        private void Update()
        {
            if (_delayRecycle.Count > 0)
            {
                for (var i = 0; i < _delayRecycle.Count; i++)
                {
                    _delayRecycle[i].DelayTime -= Time.deltaTime;
                    if (_delayRecycle[i].DelayTime < 0)
                    {
                        _delayRecycle[i].GameObjectRecycle.Recycle();
                    }
                }
            }
        }
        ...
        {
            instance._delayRecycle.Add(new ObjectAndDelayRecycle() {GameObjectRecycle = gameObject, DelayTime = delay});
        }
        ...
        public static GameObject Spawn(GameObject prefab, Transform parent, Vector3 position, Quaternion rotation, bool activeOnSpawned = true)
        {
